//
//  MineTripSliderViewController.m
//  MineTrip
//
...
#define kMineTripDeviceWidth [UIScreen mainScreen].bounds.size.width
#define kMineTripDeviceHeight [UIScreen mainScreen].bounds.size.height

static NSTimeInterval const kAnimationDuration = 0.3;

#pragma mark ---------- left config -----------
static CGFloat const leftShowWidth = 240.f;
static CGFloat const leftScale = 1.f;
...
typedef NS_ENUM(NSUInteger, MineTripDragDirection){
    MineTripDragDirectionNone = 0,
    MineTripDragDirectionLeft, // 左侧界面相关  show the leftView or hide
    MineTripDragDirectionRight,
};

@interface MineTripSliderViewController () <UIGestureRecognizerDelegate>
{
    ...
    MineTripDragDirection _dragDirection;
...
@implementation MineTripSliderViewController
/**左,右侧滑菜单初始化*/
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    ...
    // 判断点击拖动手势是否在允许拖动范围内
    if ([gestureRecognizer locationInView:_mainContainerView].x < leftDragbleWidth || [gestureRecognizer locationInView:_mainContainerView].x > kMineTripDeviceWidth-rightDragbleWidth) {
        return YES;
    }
    return NO;
}
    ...
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (!_isLeftShow&&!_isRightShow) {
                if (point.x<leftDragbleWidth||point.x>kMineTripDeviceWidth-rightDragbleWidth) {
                    _canDrag = YES;
                }else{
                    _canDrag = NO;
                }
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            if (!_isLeftShow&&!_isRightShow) {
                if (_dragDirection == MineTripDragDirectionNone) {
                    if (move_length>0) {
                        _dragDirection = MineTripDragDirectionLeft;
                        _leftContainerView.hidden = NO;
                        _rightContainerView.hidden = YES;
                    }else{
                        _dragDirection = MineTripDragDirectionRight;
                        _leftContainerView.hidden = YES;
                        _rightContainerView.hidden = NO;
                    }
                }
                ...
                switch (_dragDirection) {
                    case MineTripDragDirectionLeft:
                    {
                        if (!_canShowLeft) {
                            break;
                        }
                        
                        if (_startDragPoint.x>kMineTripDeviceWidth-rightDragbleWidth||main_x+move_length<0) {
                            // 防止拖拉左侧区域可以拖出右侧界面
                            break;
                        }
                        ...
                        break;
                    case MineTripDragDirectionRight:
                    {
                        if (!_canShowRight) {
                            break;
                        }
                ...
                if (_dragDirection == MineTripDragDirectionNone) {
                    _dragDirection = MineTripDragDirectionNone;
                }
...
- (NSTimeInterval)calculateAnimationDurationIsShow:(BOOL)isShow
{
    ...
    if (main_x>0) {
        // left
        CGFloat left_scale = _leftContainerView.frame.size.width/kMineTripDeviceWidth;
        if (isShow) {
        ...
        // right
        CGFloat right_scale = _rightContainerView.frame.size.width/kMineTripDeviceWidth;
        if (isShow) {
